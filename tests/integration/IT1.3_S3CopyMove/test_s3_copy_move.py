...
Description: IT1.3: S3 copy/move (env-gated).
...
def test_s3_copy_move(s3_env) -> None:
    ...
    backend.copy_path("/a.txt", "/b.txt", overwrite=True)
    backend.move_path("/b.txt", "/c.txt", overwrite=True)
    assert backend.read_bytes("/c.txt") == b"x"

    backend.delete_path("/a.txt", missing_ok=True)
    backend.delete_path("/c.txt", missing_ok=True)
