...
Description: IT1.16: Google Drive copy/move (env-gated).
...
def test_gdrive_copy_move(gdrive_env) -> None:
    ...
    src = f"/cloud_dog_test_{uuid.uuid4().hex}.txt"
    dst = f"/cloud_dog_test_{uuid.uuid4().hex}.txt"
    backend.write_bytes(src, b"x", overwrite=True)
    backend.copy_path(src, dst, overwrite=True)
    backend.move_path(dst, f"{dst}.moved", overwrite=True)

    backend.delete_path(src, missing_ok=True)
    backend.delete_path(f"{dst}.moved", missing_ok=True)
