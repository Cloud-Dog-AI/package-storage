...
Description: IT1.13: Google Drive CRUD (env-gated).
...
def test_gdrive_crud(gdrive_env) -> None:
    ...
    path = f"/cloud_dog_test_{uuid.uuid4().hex}.txt"
    backend.write_bytes(path, b"hello", overwrite=True)
    assert backend.read_bytes(path) == b"hello"
    backend.delete_path(path, missing_ok=False)
