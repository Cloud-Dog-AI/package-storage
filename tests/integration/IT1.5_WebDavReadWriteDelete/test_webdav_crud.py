...
Description: IT1.5: WebDAV CRUD (env-gated).
...
def test_webdav_crud(webdav_env) -> None:
    base_url = webdav_env["TEST_WEBDAV_URL"]
    backend = WebDavStorage(
        WebDavConfig(
            base_url=base_url, username=webdav_env["TEST_WEBDAV_USERNAME"], password=webdav_env["TEST_WEBDAV_PASSWORD"]
        )
    )

    base = f"/cloud_dog_test_{uuid.uuid4().hex}"
    backend.create_dir(base, parents=True, exist_ok=True)
    backend.write_bytes(f"{base}/a.txt", b"hello", overwrite=True)
    assert backend.read_bytes(f"{base}/a.txt") == b"hello"
    backend.delete_path(f"{base}/a.txt", missing_ok=False)
